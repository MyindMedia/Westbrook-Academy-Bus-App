    ...
    <div className="max-w-md mx-auto px-4 py-3 flex justify-between items-center">
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
             <WestbrookLogo className="w-10 h-10 rounded shadow-sm" />
        </div>
        <div>
          <h1 className="text-lg font-black text-gray-900 leading-none tracking-tight">WESTBROOK <span className="text-westbrook-orange">ACADEMY</span></h1>
          <p className="text-[10px] text-gray-500 uppercase tracking-widest font-medium">Operated by LA Promise Fund</p>
        </div>
      </div>
...
const StudentCard: React.FC<StudentCardProps> = ({ student, status, onStatusChange, disabled }) => {
  ...
  return (
    <div className={`p-4 rounded-xl border mb-3 flex items-center gap-4 transition-all duration-300 ${isPresent ? 'bg-green-50 border-green-200' : 'bg-white border-gray-100 shadow-sm'}`}>
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-gray-500">Grade {student.grade} • ID: {student.id}</p>
        {/* Helper tag to show if they are a guest (added) student */}
        {student.busId === 'GUEST' && <span className="text-[10px] bg-blue-100 text-blue-700 px-1 rounded">GUEST</span>}
      </div>
      <div>
        {isPresent ? (
           <button 
             disabled={disabled}
             onClick={() => onStatusChange(StudentStatus.PENDING)} 
             className="p-2 bg-green-100 text-green-700 rounded-full disabled:opacity-50"
           >
           ...
           <button 
             disabled={disabled}
             onClick={() => onStatusChange(StudentStatus.ON_BUS)} 
             className="p-2 bg-gray-100 text-gray-400 rounded-full hover:bg-gray-200 disabled:opacity-50"
           >
...
export default function App() {
  ...
  const allManifestStudents = useMemo(() => {
    ...
    // Use masterStudentList instead of import
    const assigned = masterStudentList.filter(s => s.busId === selectedBus.id);
    
    // Filter out duplicates if guestStudents somehow contains assigned ones
  ...
  // Sync Check Effect (Polls service to see if data updated elsewhere, e.g. Dashboard)
  useEffect(() => {
      const checkSync = setInterval(async () => {
          ...
          if (service.getLastSyncTime()) {
               ...
               // Simply update local state to match service source of truth
  ...
  const handleStartTrip = async () => {
    if (selectedBus) {
      ...
      setTripActive(true);
      // Don't clear logs here, as we might have pre-checked students
      setIncidents([]);
      setReportSent(false);
      setTripSummary("");
    }
  ...
  const handleCheckIn = (studentId: string, status: StudentStatus = StudentStatus.ON_BUS) => {
    ...
    // Logic for Guest Students (Students not originally assigned to this bus)
    const isAssigned = student.busId === selectedBus?.id;
    if (!isAssigned) {
        // Check if already in our guest list
  ...
  const handleEndTrip = async () => {
    ...
    if (selectedBus) {
        const report = await generateTripReport(
            ...
            routeType, 
            logs, 
            allManifestStudents, // Use combined list
            finalStats,
            incidents
        );
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      {!selectedBus ? (
          <div className="p-6">
             ...
             <div className="flex gap-4 mb-6">
               <button 
                  ...
                  className={`flex-1 py-4 rounded-xl font-bold flex flex-col items-center gap-2 border-2 ${routeType === RouteType.AM_PICKUP ? 'border-westbrook-orange bg-orange-50 text-westbrook-orange' : 'border-transparent bg-white text-gray-400'}`}
                >
                  <div className="p-2 bg-white rounded-full shadow-sm"><Users size={20} /></div>
                  AM Pickup
               </button>
               <button 
                  onClick={() => setRouteType(RouteType.PM_DROPOFF)}
                  className={`flex-1 py-4 rounded-xl font-bold flex flex-col items-center gap-2 border-2 ${routeType === RouteType.PM_DROPOFF ? 'border-westbrook-orange bg-orange-50 text-westbrook-orange' : 'border-transparent bg-white text-gray-400'}`}
                >
                   <div className="p-2 bg-white rounded-full shadow-sm"><LogOut size={20} /></div>
                   PM Dropoff
               </button>
             </div>
             ...
             <div className="space-y-4">
               {BUSES.map(bus => (
                 <button
                   ...
                   className="w-full bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4 hover:border-westbrook-blue transition-colors text-left"
                 >
                   <div className="w-12 h-12 bg-blue-50 text-westbrook-blue rounded-full flex items-center justify-center font-bold">
                     {bus.id.split('-')[1]}
                   </div>
                   <div className="flex-1">
                     ...
                     <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
                       <MapPin size={10} /> 
                       {routeType === RouteType.AM_PICKUP ? `${bus.endpointAddress} -> Academy` : `Academy -> ${bus.endpointAddress}`}
                     </p>
                   </div>
          ...
          /* VIEW 2: ROUTE DASHBOARD (PRE-TRIP & ACTIVE) */
          <div className="max-w-md mx-auto p-4">
             {/* Top Bar: Route Info & Back Button */}
             <div className="flex items-center justify-between mb-4">
               <div className="flex items-center gap-3">
                 <button 
                    onClick={() => {
                        if (tripActive) {
                            if(window.confirm("End current trip?")) handleEndTrip();
                        } else {
                            setSelectedBus(null);
                        }
                    }} 
                    className="p-2 bg-white rounded-lg shadow-sm text-gray-500"
                 >
                   <ChevronRight className="transform rotate-180" size={20} />
                 </button>
                 <div>
                    <h2 className="font-bold text-gray-800 leading-tight">{selectedBus.name}</h2>
                    <p className="text-xs text-gray-500">{routeType === RouteType.AM_PICKUP ? 'Morning Route' : 'Afternoon Route'}</p>
                 </div>
               </div>
               
               {/* Timer / Status */}
               <div className={`px-3 py-1 rounded-full text-xs font-mono font-bold flex items-center gap-2 ${tripActive ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>
                  {tripActive ? <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" /> : <Square size={8} fill="currentColor" />}
                  {tripActive ? formatDuration(elapsedTime) : 'READY'}
               </div>
             </div>

             {/* Action Bar (Start/End) */}
                ...
                {!tripActive ? (
                     ...
                     <div className="flex gap-2">
                        <button
                            ...
                            className="bg-red-50 text-red-600 px-4 rounded-2xl font-bold flex flex-col items-center justify-center border border-red-100"
                        >
                        ...
                        <button
                            ...
                            className="flex-1 bg-red-600 text-white py-4 rounded-2xl shadow-lg shadow-red-200 font-bold text-lg flex items-center justify-center gap-2 active:scale-95 transition-transform"
                        >
                            <Square fill="currentColor" size={16} /> END TRIP
                        </button>
                     </div>
                )}
             </div>
             ...
             <div className="flex gap-2 mb-4">
               <div className="flex-1 bg-white flex items-center px-4 rounded-xl shadow-sm border border-gray-100">
                 ...
                 {manualId && (
                    <button 
                        onClick={() => { handleCheckIn(manualId); setManualId(''); }}
                        className="text-westbrook-blue font-bold text-xs"
                    >
                        ADD
                    </button>
                 )}
               ...
               <button 
                 ...
                 className="bg-gray-800 text-white p-3 rounded-xl shadow-lg active:scale-95 transition-transform"
               >
                 <QrCode size={24} />
               </button>
             </div>
             ...
             <div className="flex justify-between items-center mb-2 px-1">
                <h3 className="font-bold text-gray-700 text-sm uppercase">Passenger Manifest</h3>
                <span className="text-xs font-bold bg-gray-200 px-2 py-1 rounded-full text-gray-600">
                    {logs.filter(l => l.status === StudentStatus.ON_BUS).length} / {allManifestStudents.length} On Board
                </span>
             </div>
             ...
             <div className="pb-24">
                {allManifestStudents.length === 0 ? (
                    <div className="text-center py-10 text-gray-400 bg-white rounded-xl border border-dashed">
                        ...
                        <p className="text-xs">Use search to add guests.</p>
                    </div>
                ) : (
                    allManifestStudents.map(student => {
                        const log = logs.find(l => l.studentId === student.id);
                        return (
                            <StudentCard 
                                key={student.id} 
                                student={student}
                                status={log ? log.status : StudentStatus.PENDING}
